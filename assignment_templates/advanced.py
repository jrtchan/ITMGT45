...
def relationship_status(from_member, to_member, social_graph):
    ...
    if follower and followed_by == True:
        return 'friends'
...
def tic_tac_toe(board):
    ...
    top_left_diagonal = [board[x][x] for x in range(len(board))]
    winner = winning_line(top_left_diagonal)
    if winner:    
        return winner

    bottom_left_diagonal = [board[x][(-1)*(x+1)] for x in range(len(board))]
    winner = winning_line(bottom_left_diagonal)
    if winner:
        return winner
